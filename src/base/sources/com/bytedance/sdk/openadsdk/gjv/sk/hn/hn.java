package com.bytedance.sdk.openadsdk.gjv.sk.hn;

import com.bytedance.sdk.component.utils.apu;
import org.json.JSONObject;

/* JADX INFO: compiled from: r8-map-id-1868b3f846f91b929d17a1f0de6da199bc8101b6e9bb94a36f131322636ef84b */
/* JADX INFO: loaded from: classes.dex */
public class hn extends qor {
    private int gjv;

    /* JADX INFO: renamed from: hn, reason: collision with root package name */
    private long f14467hn;
    private long hnj;
    private int qor;

    public void hn(long j10) {
        this.f14467hn = j10;
    }

    public void hnj(long j10) {
        this.hnj = j10;
    }

    public void hn(int i10) {
        this.gjv = i10;
    }

    public void hnj(int i10) {
        this.qor = i10;
    }

    @Override // com.bytedance.sdk.openadsdk.gjv.sk.hn.qor
    public void hnj(JSONObject jSONObject) {
        if (jSONObject == null) {
            return;
        }
        try {
            jSONObject.put("buffers_time", this.hnj);
            jSONObject.put("total_duration", this.f14467hn);
            jSONObject.put("vbtt_skip_type", this.qor);
            jSONObject.put("skip_reason", this.gjv);
        } catch (Throwable th2) {
            apu.qor("EndcardSkipModel", th2.getMessage());
        }
    }
}
