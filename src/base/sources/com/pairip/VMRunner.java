...
public class VMRunner {
    ...
    private static int versionCode = 70388;
    ...
    private static byte[] readByteCode(String vmByteCodeFile) throws IOException {
        ...
        try {
            ...
            try {
                ...
                if (isDebuggingEnabled()) {
                    Log.i(TAG, String.format("Finished loading %s (%d kB) after %d ms.", vmByteCodeFile, Integer.valueOf(size / UserVerificationMethods.USER_VERIFY_ALL), Long.valueOf(System.currentTimeMillis() - jCurrentTimeMillis)));
                }
                if (inputStream != null) {
                    inputStream.close();
                }
    ...
    private static byte[] readFullByteArrayFromStream(InputStream is, byte[] byteArray) throws IOException {
        ...
        throw new IOException("Read " + i10 + "/" + byteArray.length + " bytes.");
    }
