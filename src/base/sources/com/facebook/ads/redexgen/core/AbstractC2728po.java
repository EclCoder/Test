...
public abstract class AbstractC2728po {
    ...
    public static int A00(int expectedSize) {
        if (expectedSize < 3) {
            AbstractC2575nJ.A00(expectedSize, A07(2, 12, Sdk$SDKError.b.INVALID_LOG_ERROR_ENDPOINT_VALUE));
    ...
    public static <V> InterfaceC2409kO<Map.Entry<?, V>, V> A04() {
        BJ bj2 = BJ.A03;
        if (A01[6].charAt(13) == 's') {
            throw new RuntimeException();
        }
    ...
    public static String A08(Map<?, ?> map) {
        StringBuilder sbAppend = AbstractC2594nc.A01(map.size()).append('{');
        boolean z10 = true;
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            if (!z10) {
                ...
                sbAppend.append(A07(0, 2, Sdk$SDKError.b.TPAT_ERROR_VALUE));
            }
            z10 = false;
            sbAppend.append(entry.getKey()).append('=').append(entry.getValue());
        }
        StringBuilder sb2 = sbAppend.append('}');
        return sb2.toString();
    }

    public static boolean A0A(@CheckForNull Map<?, ?> map, Object value) {
        return AbstractC2688p9.A0D(A03(map.entrySet().iterator()), value);
    }
