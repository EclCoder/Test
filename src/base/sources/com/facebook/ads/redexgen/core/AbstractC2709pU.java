...
public abstract class AbstractC2709pU {
    ...
    public static <E> ArrayList<E> A05(Iterator<? extends E> elements) {
        ...
        AbstractC2688p9.A0A(list, elements);
        return list;
    }

    public static boolean A06(@CheckForNull List<?> thisList, Object other) {
        if (other == AbstractC2428ki.A04(thisList)) {
            return true;
        }
        ...
        if (z10 && (list instanceof RandomAccess)) {
            for (int i10 = 0; i10 < size; i10++) {
                if (!CB.A01(thisList.get(i10), list.get(i10))) {
                    return false;
                }
            }
            return true;
        }
        return AbstractC2688p9.A0F(thisList.iterator(), list.iterator());
    }
}
