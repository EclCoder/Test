...
public abstract class AbstractC2428ki {
    ...
    public static int A00(int index, int size) {
        return A02(index, size, A06(167, 5, Sdk$SDKError.b.ASSET_FAILED_MAX_SPACE_EXCEEDED_VALUE));
    }

    public static int A01(int index, int size) {
        return A03(index, size, A06(167, 5, Sdk$SDKError.b.ASSET_FAILED_MAX_SPACE_EXCEEDED_VALUE));
    }
    ...
    public static String A07(int start, int end, int size) {
        ...
        return (end < 0 || end > size) ? A09(end, size, A06(105, 9, 59)) : Strings.A02(A06(Sdk$SDKError.b.ASSET_WRITE_ERROR_VALUE, 53, Sdk$SDKError.b.INVALID_RI_ENDPOINT_VALUE), Integer.valueOf(end), Integer.valueOf(start));
    }

    public static String A08(int index, int size, String desc) {
        if (index < 0) {
            return Strings.A02(A06(77, 28, Sdk$SDKError.b.ASSET_WRITE_ERROR_VALUE), desc, Integer.valueOf(index));
        }
        if (size >= 0) {
    ...
    public static void A0G(boolean expression, String errorMessageTemplate, int p10, int p11) {
        if (!expression) {
            throw new IllegalArgumentException(Strings.A02(errorMessageTemplate, Integer.valueOf(p10), Integer.valueOf(p11)));
        }
    }

    public static void A0H(boolean expression, String errorMessageTemplate, long p10) {
        if (!expression) {
            throw new IllegalArgumentException(Strings.A02(errorMessageTemplate, Long.valueOf(p10)));
        }
    }
}
