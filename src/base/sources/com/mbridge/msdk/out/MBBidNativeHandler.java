...
public class MBBidNativeHandler extends a implements BaseExtraInterfaceForHandler {
    ...
    public String buildTemplateString() {
        try {
            ...
            StringBuffer stringBuffer = new StringBuffer();
            Iterator<NativeListener.Template> it = this.templateList.iterator();
            while (true) {
                boolean zHasNext = it.hasNext();
                String str = UHbHibBvYxKnPE.hZmwT;
                if (!zHasNext) {
                    return "[{$native_info}]".replace("{$native_info}", stringBuffer.subSequence(0, stringBuffer.lastIndexOf(str)));
                }
                NativeListener.Template next = it.next();
                stringBuffer.append("{\"id\":");
                stringBuffer.append(next.getId() + str);
                stringBuffer.append("\"ad_num\":");
