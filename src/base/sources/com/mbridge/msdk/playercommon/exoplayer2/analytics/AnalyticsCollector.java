...
public class AnalyticsCollector implements Player.EventListener, MetadataOutput, AudioRendererEventListener, VideoRendererEventListener, MediaSourceEventListener, BandwidthMeter.EventListener, DefaultDrmSessionEventListener {
    ...
    protected AnalyticsListener.EventTime generateEventTime(int i10, MediaSource.MediaPeriodId mediaPeriodId) {
        ...
        } else if (i10 >= currentTimeline.getWindowCount() || (mediaPeriodId != null && mediaPeriodId.isAd())) {
            defaultPositionMs = 0;
        } else {
            defaultPositionMs = currentTimeline.getWindow(i10, this.window).getDefaultPositionMs();
        }
        return new AnalyticsListener.EventTime(jElapsedRealtime, currentTimeline, i10, mediaPeriodId, defaultPositionMs, this.player.getCurrentPosition(), this.player.getBufferedPosition() - this.player.getContentPosition());
    }
