...
final class ExoPlayerImplInternal implements Handler.Callback, MediaPeriod.Callback, TrackSelector.InvalidationListener, MediaSource.SourceInfoRefreshListener, DefaultMediaClock.PlaybackParameterListener, PlayerMessage.Sender {
    ...
    private boolean isTimelineReady() {
        ...
        if (mediaPeriodHolder != null) {
            return mediaPeriodHolder.prepared || mediaPeriodHolder.info.f31947id.isAd();
        }
        return false;
    }
    ...
    */
    ...
    private void maybeUpdateLoadingPeriod() {
        ...
        if (this.queue.shouldLoadNextMediaPeriod()) {
            ...
            this.queue.enqueueNextMediaPeriod(this.rendererCapabilities, this.trackSelector, this.loadControl.getAllocator(), this.mediaSource, this.playbackInfo.timeline.getPeriod(nextMediaPeriodInfo.f31947id.periodIndex, this.period, true).uid, nextMediaPeriodInfo).prepare(this, nextMediaPeriodInfo.startPositionUs);
            setIsLoading(true);
        }
    }
    ...
    private void seekToCurrentPosition(boolean z10) throws ExoPlaybackException {
        MediaSource.MediaPeriodId mediaPeriodId = this.queue.getPlayingPeriod().info.f31947id;
    ...
    /* JADX WARN: Code duplicated, block: B:49:0x00d7 A[DONT_GENERATE] */
    ...
    private void seekToInternal(SeekPosition seekPosition) {
        ...
        try {
            if (this.mediaSource != null && this.pendingPrepareCount <= 0) {
                if (jLongValue == C.TIME_UNSET) {
                    ...
                    if (mediaPeriodId.equals(this.playbackInfo.periodId)) {
                        ...
                        if (C.usToMs(adjustedSeekPositionUs) == C.usToMs(this.playbackInfo.positionUs)) {
                            playbackInfoFromNewPosition = this.playbackInfo.fromNewPosition(mediaPeriodId, this.playbackInfo.positionUs, j10);
                            if (z10) {
                                return;
                            } else {
                                return;
                ...
                playbackInfoFromNewPosition = this.playbackInfo.fromNewPosition(mediaPeriodId, j11, j10);
            }
            this.pendingInitialSeekPosition = seekPosition;
            j11 = jLongValue;
            playbackInfoFromNewPosition = this.playbackInfo.fromNewPosition(mediaPeriodId, j11, j10);
        } finally {
            this.playbackInfo = this.playbackInfo.fromNewPosition(mediaPeriodId, jLongValue, j10);
            if (z10) {
    ...
    private boolean shouldKeepPeriodHolder(MediaSource.MediaPeriodId mediaPeriodId, long j10, MediaPeriodHolder mediaPeriodHolder) {
        if (!mediaPeriodId.equals(mediaPeriodHolder.info.f31947id) || !mediaPeriodHolder.prepared) {
            return false;
        }
        this.playbackInfo.timeline.getPeriod(mediaPeriodHolder.info.f31947id.periodIndex, this.period);
    ...
    private void updatePeriods() throws ExoPlaybackException {
        ...
        while (this.playWhenReady && playingPeriod != readingPeriod && this.rendererPositionUs >= playingPeriod.next.rendererPositionOffsetUs) {
            ...
            this.playbackInfo = playbackInfo.fromNewPosition(mediaPeriodInfo.f31947id, mediaPeriodInfo.startPositionUs, mediaPeriodInfo.contentPositionUs);
