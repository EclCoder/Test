...
public class H264TrackImpl extends AbstractH26XTrack {
    ...
    public class SEIMessage {
        ...
        public SEIMessage(InputStream inputStream, SeqParameterSet seqParameterSet) throws IOException {
            ...
            int i10 = 0;
            while (i10 < iAvailable) {
                this.payloadType = z10 ? 1 : 0;
                this.payloadSize = z10 ? 1 : 0;
                ...
                int i14 = this.payloadSize + i13;
                this.payloadSize = i14;
                if (iAvailable - i10 < i14) {
                    i10 = iAvailable;
                    ...
                    if (vUIParameters == null || (vUIParameters.nalHRDParams == null && vUIParameters.vclHRDParams == null && !vUIParameters.pic_struct_present_flag)) {
                        for (int i15 = 0; i15 < this.payloadSize; i15++) {
                            inputStream.read();
                            i10++;
                        }
                    } else {
                        ...
                        if (seqParameterSet.vuiParams.pic_struct_present_flag) {
                            int u10 = cAVLCReader.readU(4, "SEI: pic_struct");
                            this.pic_struct = u10;
                            switch (u10) {
                                case 3:
                                case 4:
                            ...
                            for (?? r10 = z10; r10 < c10; r10++) {
                                boolean bool = cAVLCReader.readBool("pic_timing SEI: clock_timestamp_flag[" + r10 + "]");
                    ...
                    for (int i16 = 0; i16 < this.payloadSize; i16++) {
                        inputStream.read();
                        i10++;
                    }
                }
    ...
    public H264TrackImpl(DataSource dataSource, String str, long j10, int i10) throws IOException {
        super(dataSource);
    ...
    private int calcPOC0(H264NalUnitHeader h264NalUnitHeader, SliceHeader sliceHeader) {
        int i10;
        int i11 = sliceHeader.pic_order_cnt_lsb;
        int i12 = 1 << (sliceHeader.sps.log2_max_pic_order_cnt_lsb_minus4 + 4);
        int i13 = this.prevPicOrderCntLsb;
        if (i11 >= i13 || i13 - i11 < i12 / 2) {
            i10 = (i11 <= i13 || i11 - i13 <= i12 / 2) ? this.prevPicOrderCntMsb : this.prevPicOrderCntMsb - i12;
        } else {
            i10 = this.prevPicOrderCntMsb + i12;
        }
        if (h264NalUnitHeader.nal_ref_idc != 0) {
            this.prevPicOrderCntMsb = i10;
            this.prevPicOrderCntLsb = i11;
        }
        return i10 + i11;
    }

    private int calcPOC1(int i10, H264NalUnitHeader h264NalUnitHeader, SliceHeader sliceHeader) {
        int i11;
        int i12 = 0;
        if (sliceHeader.sps.num_ref_frames_in_pic_order_cnt_cycle == 0) {
            i10 = 0;
        }
        if (h264NalUnitHeader.nal_ref_idc == 0 && i10 > 0) {
            i10--;
        }
        int i13 = 0;
        int i14 = 0;
        while (true) {
            ...
            i11 = seqParameterSet.num_ref_frames_in_pic_order_cnt_cycle;
            if (i13 >= i11) {
                break;
            }
            i14 += seqParameterSet.offsetForRefFrame[i13];
            i13++;
        }
        if (i10 > 0) {
            int i15 = i10 - 1;
            ...
            int i18 = i16 * i14;
            while (i12 <= i17) {
                i18 += sliceHeader.sps.offsetForRefFrame[i12];
                i12++;
            }
            i12 = i18;
        }
        if (h264NalUnitHeader.nal_ref_idc == 0) {
            i12 += sliceHeader.sps.offset_for_non_ref_pic;
        }
        return i12 + sliceHeader.delta_pic_order_cnt_0;
    }

    private int calcPOC2(int i10, H264NalUnitHeader h264NalUnitHeader, SliceHeader sliceHeader) {
        return h264NalUnitHeader.nal_ref_idc == 0 ? (i10 * 2) - 1 : i10 * 2;
    }

    private int calcPoc(int i10, H264NalUnitHeader h264NalUnitHeader, SliceHeader sliceHeader) {
        int i11 = sliceHeader.sps.pic_order_cnt_type;
        if (i11 == 0) {
            return calcPOC0(h264NalUnitHeader, sliceHeader);
        }
        return i11 == 1 ? calcPOC1(i10, h264NalUnitHeader, sliceHeader) : calcPOC2(i10, h264NalUnitHeader, sliceHeader);
    }
    ...
    private void configureFramerate() {
        if (this.determineFrameRate) {
            ...
            long j10 = vUIParameters.time_scale >> 1;
            this.timescale = j10;
            int i10 = vUIParameters.num_units_in_tick;
            this.frametick = i10;
            if (j10 == 0 || i10 == 0) {
    ...
    private void createSample(List<ByteBuffer> list) {
        ...
        while (it.hasNext()) {
            ...
            int i11 = nalUnitHeader.nal_unit_type;
        ...
        SliceHeader sliceHeader = new SliceHeader(AbstractH26XTrack.cleanBuffer(new ByteBufferBackedInputStream(list.get(list.size() - 1))), this.spsIdToSps, this.ppsIdToPps, z10);
        if (h264NalUnitHeader.nal_ref_idc == 0) {
        ...
        Sample sampleCreateSampleObject = createSampleObject(list);
        list.clear();
        ...
        SeqParameterSet seqParameterSet = sliceHeader.sps;
        int i12 = seqParameterSet.pic_order_cnt_type;
        if (i12 == 0) {
            int i13 = 1 << (seqParameterSet.log2_max_pic_order_cnt_lsb_minus4 + 4);
            int i14 = sliceHeader.pic_order_cnt_lsb;
            int i15 = this.prevPicOrderCntLsb;
            if (i14 >= i15 || i15 - i14 < i13 / 2) {
                i10 = (i14 <= i15 || i14 - i15 <= i13 / 2) ? this.prevPicOrderCntMsb : this.prevPicOrderCntMsb - i13;
            } else {
                i10 = this.prevPicOrderCntMsb + i13;
            }
            this.pictureOrderCounts = Mp4Arrays.copyOfAndAppend(this.pictureOrderCounts, i10 + i14);
            this.prevPicOrderCntLsb = i14;
            this.prevPicOrderCntMsb = i10;
        } else {
            if (i12 == 1) {
        ...
        this.sdtp.add(entry);
        this.frameNrInGop++;
        this.samples.add(sampleCreateSampleObject);
        if (z10) {
    ...
    public static H264NalUnitHeader getNalUnitHeader(ByteBuffer byteBuffer) {
        ...
        h264NalUnitHeader.nal_ref_idc = (b10 >> 5) & 3;
        h264NalUnitHeader.nal_unit_type = b10 & 31;
    ...
    private void handlePPS(ByteBuffer byteBuffer) throws IOException {
        ...
        PictureParameterSet pictureParameterSet = PictureParameterSet.read(byteBufferBackedInputStream);
        if (this.firstPictureParameterSet == null) {
            this.firstPictureParameterSet = pictureParameterSet;
        }
        this.currentPictureParameterSet = pictureParameterSet;
        byte[] array = AbstractH26XTrack.toArray((ByteBuffer) byteBuffer.rewind());
        byte[] bArr = this.ppsIdToPpsBytes.get(Integer.valueOf(pictureParameterSet.pic_parameter_set_id));
        ...
        this.ppsIdToPpsBytes.put(Integer.valueOf(pictureParameterSet.pic_parameter_set_id), array);
        this.ppsIdToPps.put(Integer.valueOf(pictureParameterSet.pic_parameter_set_id), pictureParameterSet);
    }

    private void handleSPS(ByteBuffer byteBuffer) throws IOException {
        InputStream inputStreamCleanBuffer = AbstractH26XTrack.cleanBuffer(new ByteBufferBackedInputStream(byteBuffer));
        inputStreamCleanBuffer.read();
        SeqParameterSet seqParameterSet = SeqParameterSet.read(inputStreamCleanBuffer);
        if (this.firstSeqParameterSet == null) {
            this.firstSeqParameterSet = seqParameterSet;
            configureFramerate();
        }
        this.currentSeqParameterSet = seqParameterSet;
        byte[] array = AbstractH26XTrack.toArray((ByteBuffer) byteBuffer.rewind());
        byte[] bArr = this.spsIdToSpsBytes.get(Integer.valueOf(seqParameterSet.seq_parameter_set_id));
        if (bArr != null && !Arrays.equals(bArr, array)) {
        ...
        this.spsIdToSpsBytes.put(Integer.valueOf(seqParameterSet.seq_parameter_set_id), array);
        this.spsIdToSps.put(Integer.valueOf(seqParameterSet.seq_parameter_set_id), seqParameterSet);
    }
    ...
    private boolean readSamples(AbstractH26XTrack.LookAhead lookAhead) throws IOException {
        ...
        while (true) {
            ...
            if (byteBufferFindNextNal != null) {
                H264NalUnitHeader nalUnitHeader = getNalUnitHeader(byteBufferFindNextNal);
                int i10 = nalUnitHeader.nal_unit_type;
                switch (i10) {
                    case 1:
                    case 2:
                    case 3:
                    case 4:
                    case 5:
                        ?? r10 = new Object(byteBufferFindNextNal, nalUnitHeader.nal_ref_idc, i10) { // from class: com.googlecode.mp4parser.authoring.tracks.h264.H264TrackImpl.1FirstVclNalDetector
                            ...
                            {
                                SliceHeader sliceHeader = new SliceHeader(AbstractH26XTrack.cleanBuffer(H264TrackImpl.this.new ByteBufferBackedInputStream(byteBufferFindNextNal)), H264TrackImpl.this.spsIdToSps, H264TrackImpl.this.ppsIdToPps, i10 == 5);
                                ...
                                this.nal_ref_idc = i;
                                this.pic_order_cnt_type = H264TrackImpl.this.spsIdToSps.get(Integer.valueOf(H264TrackImpl.this.ppsIdToPps.get(Integer.valueOf(i11)).seq_parameter_set_id)).pic_order_cnt_type;
                            ...
                            boolean isFirstInNew(C1FirstVclNalDetector c1FirstVclNalDetector2) {
                                ...
                                boolean z11;
                                boolean z12;
                                if (c1FirstVclNalDetector2.frame_num != this.frame_num || c1FirstVclNalDetector2.pic_parameter_set_id != this.pic_parameter_set_id || (z10 = c1FirstVclNalDetector2.field_pic_flag) != this.field_pic_flag) {
                                    return true;
                                }
                                if ((z10 && c1FirstVclNalDetector2.bottom_field_flag != this.bottom_field_flag) || c1FirstVclNalDetector2.nal_ref_idc != this.nal_ref_idc) {
                                    return true;
                                }
                                int i11 = c1FirstVclNalDetector2.pic_order_cnt_type;
                                if (i11 == 0 && this.pic_order_cnt_type == 0 && (c1FirstVclNalDetector2.pic_order_cnt_lsb != this.pic_order_cnt_lsb || c1FirstVclNalDetector2.delta_pic_order_cnt_bottom != this.delta_pic_order_cnt_bottom)) {
                                    return true;
                                }
                                if (!(i11 == 1 && this.pic_order_cnt_type == 1 && (c1FirstVclNalDetector2.delta_pic_order_cnt_0 != this.delta_pic_order_cnt_0 || c1FirstVclNalDetector2.delta_pic_order_cnt_1 != this.delta_pic_order_cnt_1)) && (z11 = c1FirstVclNalDetector2.idrPicFlag) == (z12 = this.idrPicFlag)) {
                                    return z11 && z12 && c1FirstVclNalDetector2.idr_pic_id != this.idr_pic_id;
                                }
                                return true;
                            }
                        };
                        if (c1FirstVclNalDetector != 0 && c1FirstVclNalDetector.isFirstInNew(r10)) {
                    ...
                    case 6:
                        ...
                        this.seiMessage = new SEIMessage(AbstractH26XTrack.cleanBuffer(new ByteBufferBackedInputStream(byteBufferFindNextNal)), this.currentSeqParameterSet);
    ...
    private boolean readVariables() {
        ...
        int subHeight = seqParameterSet.frame_mbs_only_flag ? 1 : 2;
        this.height = (seqParameterSet.pic_height_in_map_units_minus1 + 1) * 16 * subHeight;
        if (seqParameterSet.frame_cropping_flag) {
            if ((!seqParameterSet.residual_color_transform_flag ? seqParameterSet.chroma_format_idc.getId() : 0) != 0) {
                subWidth = this.firstSeqParameterSet.chroma_format_idc.getSubWidth();
                subHeight *= this.firstSeqParameterSet.chroma_format_idc.getSubHeight();
            } else {
                subWidth = 1;
            }
            int i10 = this.width;
            SeqParameterSet seqParameterSet2 = this.firstSeqParameterSet;
            this.width = i10 - (subWidth * (seqParameterSet2.frame_crop_left_offset + seqParameterSet2.frame_crop_right_offset));
            this.height -= subHeight * (seqParameterSet2.frame_crop_top_offset + seqParameterSet2.frame_crop_bottom_offset);
        }
        return true;
    }
    ...
    public void calcCtts() {
        ...
        int i11 = 0;
        int i12 = 0;
        while (i11 < this.pictureOrderCounts.length) {
            int i13 = Integer.MAX_VALUE;
            int i14 = 0;
            for (int iMax = Math.max(0, i11 - 128); iMax < Math.min(this.pictureOrderCounts.length, i11 + 128); iMax++) {
                int i15 = this.pictureOrderCounts[iMax];
                if (i15 > i10 && i15 < i13) {
                    i14 = iMax;
                    i13 = i15;
                }
        ...
        while (true) {
            ...
            if (i17 >= iArr2.length) {
                ...
                this.ctts.add(new CompositionTimeToSample.Entry(1, iArr2[i17] - i17));
                i17++;
            }
        }
    }
