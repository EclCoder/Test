...
public class MultiplyTimeScaleTrack implements Track {
    ...
    public MultiplyTimeScaleTrack(Track track, int i10) {
        this.source = track;
        this.timeScaleFactor = i10;
    }

    static List<CompositionTimeToSample.Entry> adjustCtts(List<CompositionTimeToSample.Entry> list, int i10) {
        if (list == null) {
            return null;
        }
        ArrayList arrayList = new ArrayList(list.size());
        for (CompositionTimeToSample.Entry entry : list) {
            arrayList.add(new CompositionTimeToSample.Entry(entry.getCount(), entry.getOffset() * i10));
        }
        return arrayList;
    }
    ...
    public long[] getSampleDurations() {
        ...
        for (int i10 = 0; i10 < this.source.getSampleDurations().length; i10++) {
            jArr[i10] = this.source.getSampleDurations()[i10] * ((long) this.timeScaleFactor);
        }
        return jArr;
    }
